...
/*
u32 crc32(const void *buf, unsigned int size)
{
        const u8 *p;
        u32 crc;
...
*/
static unsigned int gCRC32 = 0;
static unsigned int gOTACRC32 = 0;
...
int CRC32_Calculate(unsigned char *pbuff, int len)
{
	while (len > 0) 
	{
		gCRC32 = crc32_tab[(gCRC32 ^ *pbuff++) & 0xff] ^ (gCRC32 >> 8);
		len--;
	}
...
int CRC32_OTA_Calculate(unsigned char *pbuff, int len)
{
	while (len > 0) 
	{
		gOTACRC32 = crc32_tab[(gOTACRC32 ^ *pbuff++) & 0xff] ^ (gOTACRC32 >> 8);
		len--;
	}
